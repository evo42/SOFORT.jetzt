...
function done(message, resp) {
    ...
    var shippingAddress = resp.shippingAddress ?
        'shipping address: ' + JSON.stringify(toDictionary(resp.shippingAddress), undefined, 2) + '<br/>' :
        '';
    var instrument = 'instrument:' + JSON.stringify(resp.details, undefined, 2) + '<br/>';
    var method = 'method: ' + resp.methodName + '<br/>';
    var email = resp.payerEmail ?
        'email: ' + resp.payerEmail + '<br/>' :
        '';
    var phone = resp.payerPhone ?
        'phone: ' + resp.payerPhone + '<br/>' :
        '';
...
class SEPAdigital {
            ...
            if (request.canMakePayment) {
                request
                    ...
                    .then(function (result) {
                        info(result ?
                            'Can make SEPAdigital payment' :
                            'Can not make SEPAdigital payment')
                    })
                    .catch(function (err) {
                        error(err)
                    })
    ...
    buyNowClick() {
        ...
        setTimeout(function () {
            ...
            if (w3cPay) {
                ...
                if (win) {
                    ...
                    alert('Unable to open payment page SOFORT.jetzt/pay - Allow pop-ups for: https://SEPA.digital/pay')
                }
            }
        }, 1000);
        ...
        $btn
            .removeClass('fa-cash-register fa-check-circle fa-ban fa-exclamation-triangle')
            .addClass('fa-clock');
        ...
        paymentRequest
            ...
            .then((result) => {
                ...
                console.groupEnd();
                $btn
                    .removeClass('fa-cash-register fa-clock fa-ban fa-exclamation-triangle')
            ...
            .catch((err) => {
                ...
                console.groupEnd();
                $btn
                    .removeClass('fa-cash-register fa-clock fa-check-circle fa-ban')
